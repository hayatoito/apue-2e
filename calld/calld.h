...
extern char errmsg[];           /* error message string to return to
                                 * client */
...
extern enum parity {
    NONE, EVEN, ODD
} parity;                       /* specified by client */

typedef struct {                /* one Client struct per connected client */
    int fd;                     /* fd, or -1 if available */
    pid_t pid;                  /* child pid while dialing */
    uid_t uid;                  /* client's user ID */
    int childdone;              /* nonzero when SIGCHLD from dialing child
                                 * recvd: 1 means exit(0), 2 means exit(1) */
    long sysftell;              /* next line to read in Systems file */
    long foundone;              /* true if we find a matching sysfile
                                 * entry */
 ...
 */
extern Client *client;          /* ptr to malloc'ed array of Client
                                 * structs */
extern int client_size;         /* # entries in client[] array */

typedef struct {                /* everything for one entry in Systems
                                 * file */
    char *name;                 /* system name */
    char *time;                 /* (e.g., "Any") time to call (ignored) */
    char *type;                 /* (e.g., "ACU") or system name if direct
                                 * connect */
...
typedef struct {                /* everything for one entry in Devices
                                 * file */
    char *type;                 /* (e.g., "ACU") matched by type in
                                 * Systems */
    char *line;                 /* (e.g., "cua0") without preceding
                                 * "/dev/" */
    char *line2;                /* (ignored) */
...
typedef struct {                /* everything for one entry in Dialers
                                 * file */
    char *dialer;               /* matched by dialer in Devices */
    char *sub;                  /* phone number substitution string
                                 * (ignored) */
